...
from transformers import AutoModelForCausalLM, get_scheduler, AutoModelForSequenceClassification, pipeline
...
from dschat.utils.reward_module import RMGemmaRewardModule, LlamaGuardRewardModule, QualitySafetyRewardModule
...
class DeepSpeedRLHFEngine():
    ...
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path,
                 protected_model_name_or_path,
                 quality_reward_model_name_or_path, safety_reward_model_name_or_path,
                 tokenizer, reward_quality_tokenizer, reward_safety_tokenizer, protected_tokenizer,
                 args, num_total_iters):
        self.args = args
        self.num_total_iters = num_total_iters
        # this tokenizer for llamas
        self.tokenizer = tokenizer
        self.reward_safety_tokenizer = reward_safety_tokenizer
        self.protected_tokenizer = protected_tokenizer
        self.reward_quality_tokenizer = reward_quality_tokenizer
        print(f"actor tokenizer padding_side: {self.tokenizer.padding_side}")
        print(f"protected tokenizer padding_side: {self.protected_tokenizer.padding_side}")
        print(f"reward quality tokenizer padding_side: {self.reward_quality_tokenizer.padding_side}")
        print(f"reward safety tokenizer padding_side: {self.reward_safety_tokenizer.padding_side}")
        self.actor = self._init_actor(
            actor_model_name_or_path=actor_model_name_or_path, tokenizer=tokenizer, zero_stage=self.args.actor_zero_stage)
        self.protected = self._init_ref(
            actor_model_name_or_path=protected_model_name_or_path, tokenizer=protected_tokenizer, zero_stage=self.args.protected_zero_stage)
        ...
        self.critic = self._init_critic(
            critic_model_name_or_path=critic_model_name_or_path, tokenizer=tokenizer, zero_stage=self.args.critic_zero_stage)
        
        # we use an assemble of reward models
        if "RM-Gemma-2B" in quality_reward_model_name_or_path and "Llama-Guard" in safety_reward_model_name_or_path:
            self.reward_module = QualitySafetyRewardModule(
                safety_reward_module=LlamaGuardRewardModule(
                    llama_guard_model=self._init_ref(actor_model_name_or_path=safety_reward_model_name_or_path, tokenizer=reward_safety_tokenizer, zero_stage=3),
                    llama_guard_tokenizer=self.reward_safety_tokenizer,
                    z3_enabled=True
                ),
                quality_reward_module=RMGemmaRewardModule(
                    gemma_rm_pipeline=pipeline(
                        "sentiment-analysis",
                        model=quality_reward_model_name_or_path,
                        device_map='auto',
                        tokenizer=self.reward_quality_tokenizer,
                        model_kwargs={"torch_dtype": torch.bfloat16}
                    ),
                    gemma_tokenizer=self.reward_quality_tokenizer
                ),
                weight_safety=1.0,
                weight_quality=1.0
            )
        else:
            exit("Unsupported reward module.")
    ...
    def _init_actor(self, actor_model_name_or_path, tokenizer, zero_stage):
        stime = log_init("Actor")
        print(f"{actor_model_name_or_path} zero_stage: {zero_stage}")
    ...
    def _init_ref(self, actor_model_name_or_path, tokenizer, zero_stage):
        stime = log_init("Ref")
        print(f"{actor_model_name_or_path} zero_stage: {zero_stage}")
        # DS Config
        ...
        ref_model = create_hf_model(AutoModelForCausalLM,
                                    actor_model_name_or_path, tokenizer,
                                    ds_config)
    ...
    def _init_pipeline(self, reward_model_name_or_path, tokenizer, zero_stage):
        stime = log_init("Pipeline")
        print(f"{reward_model_name_or_path} zero_stage: {zero_stage}")
        ...
        reward_model = AutoModelForSequenceClassification.from_pretrained(reward_model_name_or_path)
    ...
    def _init_roberta_reward(self, actor_model_name_or_path, tokenizer):
        stime = log_init("Ref")
        # DS Config
        zero_stage = self.args.actor_zero_stage
        if zero_stage != 3:
        ...
        ref_model = create_hf_model(AutoModelForSequenceClassification,
                                    actor_model_name_or_path, tokenizer,
                                    ds_config)
    ...
    def _init_critic(self, critic_model_name_or_path, tokenizer, zero_stage):
        stime = log_init("Critic")
        print(f"{critic_model_name_or_path} zero_stage: {zero_stage}")
        ds_config = get_train_ds_config(
            offload=self.args.offload,
            dtype=self.args.dtype,
