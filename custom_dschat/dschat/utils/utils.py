...
def load_hf_tokenizer(model_name_or_path,
    ...
    if os.path.exists(model_name_or_path):
        ...
        if "gemma" in model_name_or_path or "Gemma" in model_name_or_path:
            tokenizer = AutoTokenizer.from_pretrained(
                model_name_or_path, fast_tokenizer=fast_tokenizer)
            if "RM" not in model_name_or_path:
                tokenizer.padding_side = 'right'
            print(f"#############################################################\n"
                  f"Gemma tokenizer is used, model_name_or_path: {model_name_or_path}\n"
                  f"Gemma tokenizer pad_token: {tokenizer.pad_token}\n"
                  f"#############################################################"
                  )
        elif "LlamaGuard-7b" in model_name_or_path:
            tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
            tokenizer.pad_token_id = 0
            print(f"#############################################################\n"
                  f"LlamaGuard-7b tokenizer is used, model_name_or_path: {model_name_or_path}\n"
                  f"LlamaGuard-7b tokenizer pad_token: {tokenizer.pad_token}\n"
                  f"#############################################################"
                )
        elif "Meta-Llama-Guard-2-8B" in model_name_or_path:
            tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
            tokenizer.pad_token_id = 128002
            tokenizer.padding_side = 'left'
            print(f"#############################################################\n"
                  f"Meta-Llama-Guard-2-8B tokenizer is used, model_name_or_path: {model_name_or_path}\n"
                  f"Meta-Llama-Guard-2-8B tokenizer pad_token: {tokenizer.pad_token}\n"
                  f"Meta-Llama-Guard-2-8B tokenizer padding_side: {tokenizer.padding_side}\n"
                  f"#############################################################"
                )
            ...
            if is_target:
                ...
                tokenizer.padding_side = 'right'
            print(f"#############################################################\n"
                    f"vicuna tokenizer is used, model_name_or_path: {model_name_or_path}\n"
                    f"vicuna tokenizer pad_token: {tokenizer.pad_token}\n"
                    f"vicuna tokenizer padding_side: {tokenizer.padding_side}\n"
                    f"#############################################################"
                    )
        elif "Llama-2-7b-chat-hf" in model_name_or_path and is_target:
            tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
            tokenizer.pad_token_id = 0
            tokenizer.padding_side='left'
            print(f"#############################################################\n"
                  f"Llama-2-7b-chat-hf tokenizer is used, model_name_or_path: {model_name_or_path}\n"
                  f"Llama-2-7b-chat-hf tokenizer pad_token: {tokenizer.pad_token}\n"
                  f"Llama-2-7b-chat-hf tokenizer padding_side: {tokenizer.padding_side}\n"
                  f"#############################################################"
                  )
    else:
