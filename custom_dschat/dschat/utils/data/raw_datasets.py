...
class MyDataset(Dataset):
    ...
    def __len__(self):
        return len(self.data_dict['instruction'])

    def __getitem__(self, index):
        # output = self.data_dict['output'][index]
        instruction = self.data_dict['instruction'][index]
...
class RefineDatasetForVicuna(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.train, self.eval = train_test_split(self.raw_datasets['train'],
                                                    train_size=0.9, test_size=0.1, random_state=42)
        # print('##############################################################################')
        # print(self.train[0])
        self.train = MyDataset(self.train)
        self.eval = MyDataset(self.eval)
        self.template = (
    ...
    def get_prompt(self, sample):
        ...
        conv.append_message(conv.roles[0], self.template.format(query=sample['instruction']))
        conv.append_message(conv.roles[1], None)

        return conv.get_prompt(), sample['instruction']
...
class RefineDatasetForGemma(PromptRawDataset):
    ...
    def get_prompt(self, sample):
        ...
        prompt = [
            {
                "role": "user", "content": template.format(query=sample['instruction'])
            }
        ]

        return self.tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True), sample['instruction']
