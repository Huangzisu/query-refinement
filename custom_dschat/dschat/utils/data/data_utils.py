...
def get_raw_dataset(actor_model_name_or_path, dataset_name, output_path, seed, local_rank, tokenizer):
    ...
    elif "rl_refine.json" in dataset_name:
        if "gemma" in actor_model_name_or_path:
            print("Using gemma dataset...")
            return raw_datasets.RefineDatasetForGemma(output_path, seed, local_rank,
                                                     dataset_name, tokenizer)
        elif "vicuna" in actor_model_name_or_path:
            print("Using vicuna dataset...")
            return raw_datasets.RefineDatasetForVicuna(output_path, seed, local_rank,
                                                     dataset_name)
    else:
        raise RuntimeError(
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            ...
            return self.prompt_dataset[idx]["input_ids"],self.prompt_dataset[idx]["attention_mask"], \
                self.pad_token_id, self.prompt_dataset[idx]["original_instruction"]
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(current_dataset):
            # tokenize the text
            prompt = raw_dataset.get_prompt(tmp_data)[0]
            original_instruction = raw_dataset.get_prompt(tmp_data)[1]
            #############################
            if prompt is not None:
                ...
                if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                    ...
                    prompt_token["original_instruction"] = original_instruction
...
def create_dataset(actor_model_name_or_path, local_rank, dataset_name, data_split, output_path,
                   train_phase, seed, tokenizer, end_of_conversation_token,
                   max_seq_len, rebuild):
    raw_dataset = get_raw_dataset(actor_model_name_or_path, dataset_name, output_path, seed, local_rank, tokenizer)
...
def create_prompt_dataset(actor_model_name_or_path,
                          local_rank,
                          data_path,
                          data_split,
                          output_path,
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if len(data_path) == 1:  # Single dataset.
            train_dataset, eval_dataset = create_dataset(actor_model_name_or_path,
                local_rank,
                data_path[0],
                data_split,
                output_path,
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        ...
        batch['original_instruction'] = [f[3] for f in data]
        return batch
