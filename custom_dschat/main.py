...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True)
    parser.add_argument(
        "--actor_model_tokenizer_path",
        type=str,
        help=
        "Path to pretrained tokenizer from huggingface.co/models.",
        required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=True)
    parser.add_argument(
        "--quality_reward_model_name_or_path",
        type=str,
        default=None,
        help=
        "Path to pretrained model or model identifier from huggingface.co/models.",
        required=True)
    parser.add_argument(
        "--safety_reward_model_name_or_path",
        type=str,
        default=None,
        help=
        "Path to pretrained model or model identifier from huggingface.co/models.",
    ...
    parser.add_argument(
        "--protected_zero_stage",
        type=int,
        default=0,
        help="ZeRO optimization stage for Protected model."
    )
    parser.add_argument(
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    prompt_train_dataset, _ = create_prompt_dataset(args.actor_model_name_or_path,
...
def main():
    ...
    tokenizer = load_hf_tokenizer(args.actor_model_tokenizer_path,
                                  fast_tokenizer=True,
                                  add_special_tokens=additional_special_tokens,
                                  is_target=False)
    reward_safety_tokenizer = load_hf_tokenizer(args.safety_reward_model_name_or_path,
                                  fast_tokenizer=True,
                                  add_special_tokens=additional_special_tokens)
    protected_tokenizer = load_hf_tokenizer(args.protected_model_name_or_path,
                                        fast_tokenizer=True,
                                        add_special_tokens=additional_special_tokens)
    reward_quality_tokenizer = load_hf_tokenizer(args.quality_reward_model_name_or_path,
                                        fast_tokenizer=True,
                                        add_special_tokens=additional_special_tokens)
    ...
    rlhf_engine = DeepSpeedRLHFEngine(
        ...
        protected_model_name_or_path=args.protected_model_name_or_path,
        quality_reward_model_name_or_path=args.quality_reward_model_name_or_path,
        safety_reward_model_name_or_path=args.safety_reward_model_name_or_path,
        tokenizer=tokenizer,
        reward_quality_tokenizer=reward_quality_tokenizer,
        reward_safety_tokenizer=reward_safety_tokenizer,
        protected_tokenizer=protected_tokenizer,
    ...
    print("saving steps: ", args.saving_steps)
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            out = trainer.generate_experience(batch_prompt['prompt'],
                                              ...
                                              batch_prompt['original_instruction'],
                                              step)
            ...
            if exp_dataset is not None:
                ...
                average_reward = 0
                average_kl = 0
                average_quality_reward = 0
                average_safety_reward = 0
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        actor_loss, critic_loss = trainer.train_rlhf(exp_data, step)
                        actor_loss_sum += actor_loss.item()
                        critic_loss_sum += critic_loss.item()
                        average_reward += exp_data["rewards"].mean()
                        average_kl += exp_data['kl'].mean()
                        average_quality_reward += exp_data["quality_rewards"].mean()
                        average_safety_reward += exp_data["safety_rewards"].mean()
                ...
                average_quality_reward = get_all_reduce_mean(average_quality_reward).item()
                average_safety_reward = get_all_reduce_mean(average_safety_reward).item()
                ...
                if args.enable_tensorboard and torch.distributed.get_rank(
                    ...
                    writer.add_scalar('critic_loss',
                                      critic_loss.item(),
                                      global_step=step)
                    writer.add_scalar('quality_reward',
                                      average_quality_reward / inner_iter,
                                      global_step=step)
                    writer.add_scalar('safety_reward',
                                        average_safety_reward / inner_iter,
                                        global_step=step)
                    writer.add_scalar('average_kl',
                                      average_kl,
                                      global_step=step)
                ...
                #saving in step
                # if args.output_dir is not None and step%(args.saving_steps*args.generation_batches)==0:
                #     print_rank_0('saving model '+str(step))
                #     saving_actor = convert_lora_to_linear_layer(rlhf_engine.actor)
                #     saving_critic = convert_lora_to_linear_layer(rlhf_engine.critic)
                #     if args.enable_ema:
                #         rlhf_engine.actor_ema = convert_lora_to_linear_layer(
                #             rlhf_engine.actor_ema)

                #     if torch.distributed.get_rank() == 0:
                #         save_hf_format(saving_actor,
                #                     tokenizer,
                #                     args,
                #                     sub_folder=('actor_'+str(step)))
                #         save_hf_format(saving_critic,
                #                     tokenizer,
                #                     args,
                #                     sub_folder=('critic_'+str(step)))
                #         if args.enable_ema:
                ...
                #         save_zero_three_model(saving_actor,
                #                             global_rank=args.global_rank,
                #                             save_dir=os.path.join(
                #                                 args.output_dir, 'actor_', str(step)),
                #                             zero_stage=args.actor_zero_stage)
                #         if args.enable_ema:
                ...
                #         save_zero_three_model(saving_critic,
                #                             global_rank=args.global_rank,
                #                             save_dir=os.path.join(
                #                                 args.output_dir, 'critic_', str(step)),
